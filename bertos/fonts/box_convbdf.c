...
int gen_c_source(struct font* pf, char *path)
{
	...
	"static const PROGMEM uint8_t font_%s_glyphs[] = {\n"
    };
    ...
    if (pf->offset) {
		...
		"static const PROGMEM uint16_t %s_offset[] = {\n", pf->name);
    ...
    if (pf->width) {
		...
		"static const PROGMEM uint8_t %s_width[] = {\n" , pf->name);
