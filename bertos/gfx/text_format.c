...
static int text_charWidth(int c, struct TextWidthData *twd)
{
	...
	if (bm->font->offset)
		/* Proportional font */
#if CPU_HARVARD
		glyph_width = pgm_read_byte(&bm->font->widths[index]);
#else
...
void PGM_FUNC(text_xywrap)(struct Bitmap *bm, coord_t x, coord_t y, const char *s, int lineSize /*, const char *prefix*/) {

	const char *head = s;
	int pos, lastSpace, width, height;
	
	pos = lastSpace = width = height = 0;

	while(head[pos]!=0) {

		int isLf = (head[pos]=='\n');

		if (isLf || width >= lineSize) {

			width = 0;

			// just cut any line feed character
			if (isLf || lastSpace == 0) { lastSpace = pos; }

			//if (prefix!=NULL) { printf("%s", prefix); }

			// Print out the characters BEFORE the last space character
			while(*head!=0 && lastSpace-- > 0) { 
				
				text_xyprintf(bm, x + width, y + height, 0, "%c", *head);
				width += text_widthf(bm, "%c",  *head);
			...
			// Now insert the linebreak
			height += bm->font->height;
			width = 0;
			//printf("\n");
			//LOG_INFO("--\n");


			// Jump over any found line feed characters
			if (isLf) { head++; }

			// Clear any leading space
			...
			width += text_widthf(bm, "%c",  head[pos]);

			if (width + text_widthf(bm, "%c",  head[pos+1]) >= lineSize) { width = lineSize; }
			pos++;
			//LOG_INFO("++\n");
	...
	//text_xyprintf(bm, x, y + height, 0, "%s", prefix);
	text_xyprintf(bm, x, y + height, 0, "%s", head);
}
