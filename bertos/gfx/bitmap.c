...
void gfx_blitImage(Bitmap *dst, coord_t dxmin, coord_t dymin, const Image *image)
{
	...
	#if CPU_HARVARD
		gfx_blitRaster_P(dst, dxmin, dymin, image->raster, image->width, image->height, image->stride);
	#else
		gfx_blitRaster(dst, dxmin, dymin, image->raster, image->width, image->height, image->stride);
	#endif
