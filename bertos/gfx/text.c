...
static int text_putglyph(char c, struct Bitmap *bm)
{
	...
	if (UNLIKELY(!FONT_HAS_GLYPH(bm->font, index)))
	{
		...
		//LOG_INFO("Illegal char '%c' (0x%02x)\n", index, index);
		if (FONT_HAS_GLYPH(bm->font, '?'))
			index = '?';
		else
			index = bm->font->first;
		return c;						// FIXME: Nicht klar ob sicher... 
	}
	...
	if (bm->font->offset)
	{
		/* Proportional font */
#if CPU_HARVARD
		glyph_width = pgm_read_byte(&bm->font->widths[index]); /* TODO: optimize away */
		glyph = bm->font->glyph + pgm_read_word(&bm->font->offset[index]);
#else
	...
	if (UNLIKELY(bm->styles))
	{
		...
		//LOG_INFO("G: %d %d %d %d\n", glyph_height, glyph_width, glyph_height_bytes, glyph );
		...
		for (row = 0, row_bit = 0; row < glyph_height_bytes; ++row, row_bit += 8)
		{
			...
			for (col = 0; col < glyph_width; ++col)
			{
...
#if CPU_HARVARD
				dots = pgm_read_byte(RAST_ADDR(glyph, src_col, row_bit, glyph_width));
#else
				...
				#else
					gfx_blitRaster(bm, bm->penX + col, bm->penY + row_bit, &dots, 1, MIN((uint8_t)8, (uint8_t)(glyph_height - row_bit)), 1);
					//LOG_INFO("D: %d %d %d\n", row_bit, MIN((uint8_t)8, (uint8_t)(glyph_height - row_bit)), col );
				#endif
			}
...
int text_putchar(char c, struct Bitmap *bm)
{
	...
	if (UNLIKELY(ansi_mode))
	{
		...
		case ANSI_ESC_CLEARSCREEN:
			...
			break;
		DB(default:
			LOG_INFO("Unknown ANSI esc code: %x\n", c);)
		}
		ansi_mode = false;
	}
