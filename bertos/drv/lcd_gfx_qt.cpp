	...
	bg_brush(QColor(LCD_BG_COLOR))
{
	...
	// initialize bitmap
	memset(raster, 0x01, sizeof(raster));
...
void EmulLCD::paintEvent(QPaintEvent * /*event*/)
{
	...
	//QImage img(raster, WIDTH, HEIGHT, QImage::Format_RGB888);
	...
	QImage image(128, 66, QImage::Format_ARGB32);
	image.fill(QColor(Qt::white).rgb());
	QPainter paint(&image);
	//paint.setBackgroundMode(Qt::OpaqueMode);
	//paint.setBackground(QColor(0xff, 0xff, 0xff));

	paint.drawImage(QPoint(frame_width, frame_width), img);
	//img.setPixel(x, y, qRgb(0, 0, 0));

	paint.end();
	//image.save("screen.png");		// Uncomment to write a image-file of the screen
}
