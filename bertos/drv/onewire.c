...
//#include "lcd_ks0108.h"		/* _delay_loop_2_x */

//#include "core/bit-macros.h"
//#include <mware/formatwr.h>		/* itoa_fixedpoint */


#if ONEWIRE_HOOK_SUPPORT
...
#define LOG_LEVEL         		ONEWIRE_LOG_LEVEL
#define LOG_FORMAT     			ONEWIRE_LOG_FORMAT
#include <cfg/log.h>


#define DEBUG_ECMD_OW_ROM		0
#define DEBUG_ECMD_OW_LIST 		0
#define DEBUG_OW_POLLING		0


#if DEBUG_ECMD_OW_ROM
#define OW_DEBUG_ROM(str...)	LOG_INFO("OW-ROM: " str)
#else
#define OW_DEBUG_ROM(...)		((void) 0)
#endif

#if DEBUG_ECMD_OW_LIST
#define OW_DEBUG_LIST(str...)	LOG_INFO("OW-LIST: " str)
#else
#define OW_DEBUG_LIST(...)		((void) 0)
#endif

#if DEBUG_OW_POLLING
#define OW_DEBUG_POLL(str...)	LOG_INFO("OW-POLL: " str)
#else
...
uint8_t noinline reset_onewire(uint8_t busmask)
{
  ...
  /* Sehr seltsam wird hier nicht das richtige im Debug Log ausgegeben... No IDEE !! */
  //LOG_INFO("%d %d %d \n", OW_RESET_TIMEOUT_1, OW_RESET_TIMEOUT_2, OW_RESET_TIMEOUT_3 );
  //LOG_INFO("%d %d %d \n", data1, data2, busmask );
...
int8_t noinline ow_search_rom(uint8_t busmask, uint8_t first)
{
  ...
  //LOG_INFO("ROM: %d %x", busmask, OW_ROM_SEARCH_ROM );
...
#if ONEWIRE_ECMD_LIST_POWER_SUPPORT
