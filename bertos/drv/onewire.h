...
#define OW_RESET_TIMEOUT_1	(F_CPU / 1000000UL * 480 / 4)

/* wait 90µs before sampling data for the low-pulse detection */
#define OW_RESET_TIMEOUT_2	(F_CPU / 1000000UL *  90 / 4)

/* wait 480µs - 90µs = 390µs before initiating the next onewire command */
#define OW_RESET_TIMEOUT_3	(F_CPU / 1000000UL * 390 / 4)
 ...
 * approximately 80µs */
#define OW_WRITE_0_TIMEOUT (F_CPU / 1000000UL * 84 / 4)


/* a write 1 timeslot is initiated by holding the data line low for
 * approximately 4µs and then waiting at least 60µs */
#define OW_WRITE_1_TIMEOUT_1 (F_CPU / 1000000UL *  4 / 4)
#define OW_WRITE_1_TIMEOUT_2 (F_CPU / 1000000UL * 80 / 4)
...
#define OW_READ_TIMEOUT_1 (F_CPU / 1000000UL * 1 / 4)
#define OW_READ_TIMEOUT_2 (F_CPU / 1000000UL * 14 / 4)
#define OW_READ_TIMEOUT_3 (F_CPU / 1000000UL * 65 / 4)



static __inline__ void        /* exactly 4 cycles/loop, max 2**16 loops */
_delay_loop_2( uint16_t __n )
{                                               /* cycles per loop      */
