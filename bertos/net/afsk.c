...
#define PHASE_MAX    (SAMPLEPERBIT * PHASE_BIT)				// = 64
...
{
	//AFSK_STROBE_TOGGLE ();
	//if (af->phase_acc >= 512) af->phase_acc = 0;

	af->phase_acc += SPACE_INC;
	//af->phase_acc += MARK_INC;
	af->phase_acc %= SIN_LEN;
	return sin_sample(af->phase_acc);
}
...
void afsk_init(Afsk *af, int adc_ch, int dac_ch)
{
	...
	#if CONFIG_AFSK_RXTIMEOUT != -1
	MOD_CHECK(timer);
	LOG_INFO("AFSK modcheck.. ok\n");
	#endif
	memset(af, 0, sizeof(*af));
	af->adc_ch = adc_ch;
	af->dac_ch = dac_ch;
	LOG_INFO("AFSK memset.. ok\n");
	...
	LOG_INFO("AFSK fifo.. ok\n");

	AFSK_ADC_INIT(adc_ch, af);
	LOG_INFO("AFSK adc.. ok\n");
	AFSK_DAC_INIT(dac_ch, af);
	LOG_INFO("AFSK dac.. ok\n");
